...
static int pmtrdecls(int scls) {
	...
	if (RPAREN == Token)
		return 0;
	na = 0;
	addr = (scls == CFAR || scls == CFPROTO) ? FPRMSIZE : PRMSIZE;
	for (;;) {
...
void top(void) {
	...
	case FAR:	clss = CFAR; Token = scan(); break;
